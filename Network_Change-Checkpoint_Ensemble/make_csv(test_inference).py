...
if classification == True and crop == True and RGB == True:
    train_imgs = train_imgs.map(input_tensor_crop_RGB).apply(tf.contrib.data.unbatch()).shuffle(buffer_size=100).apply(
        lambda x: make_batch(x)).batch(batch_size).repeat()
    test_imgs = test_imgs.map(input_tensor_crop_RGB).apply(tf.contrib.data.unbatch()).shuffle(buffer_size=100).apply(
        lambda x: make_batch(x)).batch(batch_size).repeat()
...
if classification == True and crop == False and resize == False and RGB == False:
    train_imgs = train_imgs.map(input_tensor_resizeX_gray).shuffle(buffer_size=100).apply(
        lambda x: make_batch(x)).batch(
        batch_size).repeat()
...
if classification == True and crop == False and resize == True and RGB == False:
    train_imgs = train_imgs.map(input_tensor_resizeO_gray).shuffle(buffer_size=100).apply(
        lambda x: make_batch(x)).batch(
        batch_size).repeat()
...
file_list = sorted(os.listdir('./checkpoints_concat_ensemble/'), key=lambda x: int(x[5:]))
...
for fn in file_list:
    ...
    saver.restore(sess, './checkpoints_concat_ensemble/{}/model.ckpt'.format(fn))
    ...
    if crop == False:
        ...
        df2.to_csv("pred_results_concat_ensemble/{}_pred.csv".format(fn), encoding='utf-8', index=False)
