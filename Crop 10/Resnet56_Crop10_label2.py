...
# Layer 갯수 : nx6+2	
# EX) 32 layers: n=5, 56 layers: n=9, 110 layers: n=18, 152 layers: n=25
...
class Resnet():
    def __init__(self, x, y, bn, classification, n):
        ...
        with tf.name_scope("layer_1"):
            net = x
            net1 = tflearn.conv_2d(net, 16, 3, regularizer='L2', weight_decay=0.0001)
            net2 = tflearn.residual_block(net1, n, 16)
            net3 = tflearn.residual_block(net2, 1, 32, downsample=True)
            net4 = tflearn.residual_block(net3, n-1, 32)
            net5 = tflearn.residual_block(net4, 1, 64, downsample=True)
            net6 = tflearn.residual_block(net5, n-1, 64)

        with tf.name_scope("layer_2"):
            net7 = tflearn.batch_normalization(net6)
            net8 = tflearn.activation(net7, 'relu')
            net9 = tflearn.global_avg_pool(net8)
            fc1 = tf.layers.flatten(net9)
...
model = Resnet(x, y, bn=True, classification=classification, n=9)

if classification is True:
    model_name = "Resnet56_classification_crop10_voting"
else:
    model_name = "Resnet56_regression_crop10_voting"
...
if classification is True:
    ...
    saver.save(sess, os.path.join(logs_path, 'Resnet56_classification_crop', model_name))
    ...
    saver.save(sess, os.path.join(logs_path, 'Resnet56_regression_crop', model_name))
