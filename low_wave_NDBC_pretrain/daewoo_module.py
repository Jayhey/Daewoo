...
def input_tensor_crop_RGB(img_path, label):
...
def input_tensor_crop_gray(img_path, label):
    label_crop = tf.one_hot(label, NUM_CLASSES)
    ...
    return tf.reshape(img_crop, [-1, 135, 135, 1]), tf.reshape(label_crop, [-1, NUM_CLASSES])

def input_tensor_resizeX_RGB(img_path, label):
    # Regression 일 때로 수정
...
def input_tensor_resizeX_gray(img_path, label):
...
def input_tensor_resizeO_RGB(img_path, label):
    label = tf.one_hot(label, NUM_CLASSES)
...
def input_tensor_resizeO_gray(img_path, label):
    label = tf.one_hot(label, NUM_CLASSES)
...
class VGG16():
    def __init__(self, x, y, bn, classification):
        ...
        if classification is True:
            ...
            self.logits = tf.layers.dense(fc3, 1, activation=None)
...
class VGG16_CONCAT():
    def __init__(self, x, y, bn, classification):
        ...
        with tf.name_scope("layer_1"):
            self.conv1 = conv2d(x, 64, batch_norm=bn)
            self.conv2 = conv2d(self.conv1, 64, batch_norm=bn)
            self.pool1 = pooling(self.conv2)

        with tf.name_scope("layer_2"):
            self.conv3 = conv2d(self.pool1, 128, batch_norm=bn)
            self.conv4 = conv2d(self.conv3, 128, batch_norm=bn)
            self.pool2 = pooling(self.conv4)

        with tf.name_scope("layer_3"):
            self.conv5 = conv2d(self.pool2, 256, batch_norm=bn)
            self.conv6 = conv2d(self.conv5, 256, batch_norm=bn)
            self.conv7 = conv2d(self.conv6, 256, batch_norm=bn)
            self.pool3 = pooling(self.conv7)

        with tf.name_scope("layer_4"):
            self.conv8 = conv2d(self.pool3, 512, batch_norm=bn)
            self.conv9 = conv2d(self.conv8, 512, batch_norm=bn)
            self.conv10 = conv2d(self.conv9, 512, batch_norm=bn)
            self.pool4 = pooling(self.conv10)

        with tf.name_scope("layer_5"):
            self.conv11 = conv2d(self.pool4, 512, batch_norm=bn)
            self.conv12 = conv2d(self.conv11, 512, batch_norm=bn)
            self.conv13 = conv2d(self.conv12, 512, batch_norm=bn)
#            pool5 = pooling(self.conv13)
        
        with tf.name_scope("GMP_GAP_CONV"):
            self.concat1 = tf.layers.flatten(tf.contrib.layers.max_pool2d(self.conv13,
                                                        (self.conv13.shape[1],self.conv13.shape[2])))
            self.concat2 = tf.layers.flatten(tf.contrib.layers.avg_pool2d(self.conv13,
                                                        (self.conv13.shape[1],self.conv13.shape[2])))
            self.concat3 = tf.layers.flatten(tf.contrib.layers.conv2d(self.conv13, 512, 
                                                                      kernel_size=(self.conv13.shape[1], self.conv13.shape[2]),
                                                                      activation_fn=None, 
                                                                      padding="Valid"))
            self.concat = tf.concat((self.concat1, self.concat2, self.concat3), axis=1)
#         with tf.name_scope("FC_layer"):
#             fc1 = tf.layers.flatten(pool5)
#             fc2 = dense(fc1, 4096, batch_norm=bn)
#             fc3 = dense(fc2, 4096, batch_norm=bn)
        ...
        self.logits = dense(self.concat, NUM_CLASSES, fn=None, batch_norm=True)
